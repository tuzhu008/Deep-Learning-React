 ...
 * 设置节点上多个样式的值。如果将值指定为(空字符串)，则取消设置相应的 style 属性。
 * style 属性接收的是一个对象
 *
...
export function setValueForStyles(node, styles) {
  ...
  for (let styleName in styles) {
    ...
    const styleValue = dangerousStyleValue( // 处理样式的值，返回处理后的值
      styleName,
      styles[styleName],
      isCustomProperty,
    );
    if (styleName === 'float') { // 重命名 float 样式名，因为此属性在 style 对象上表现为 cssFloat
      styleName = 'cssFloat';
    }
    if (isCustomProperty) {
