...
export function reconcileChildren(
  ...
  if (current === null) {
    ...
    // 如果这是一个尚未渲染的新组件，
    // 我们不会通过应用最小的副作用来更新它的子组件集。
    // 相反，我们将在子元素渲染之前将它们全部添加到子元素中。
    // 这意味着我们可以通过不跟踪副作用来优化这个调度通道。
    ...
    // let's throw it out.
    // 如果当前的子节点与正在进行的工作相同，
    // 这意味着我们还没有开始对这些子节点进行任何工作。
    // 因此，我们使用克隆算法创建所有当前子节点的副本。
    // 如果我们已经有任何进行的工作了，那么在这一点上是无效的，所以让我们把它扔掉。
...
function updateHostRoot(current, workInProgress, renderExpirationTime) {
  ...
  // 注意：React DevTools 当前依赖于名为 “element” 的属性。
  const nextChildren = nextState.element;
  if (nextChildren === prevChildren) {
    // If the state is the same as before, that's a bailout because we had
    // no work that expires at this time.
    // 如果这个 state 和以前一样，那就是跳伞（bailout），
    // 因为这个时候到期我们不需要做什么。
    resetHydrationState();
    return bailoutOnAlreadyFinishedWork(
      current,
...
function updateHostComponent(current, workInProgress, renderExpirationTime) {
  ...
  if (isDirectTextChild) {
    ...
    // 我们特殊的包装宿主节点的直接文本子节点。
    // 这是常见的情况。我们不会把它当作一个具体化的子节点来处理。
    // 我们将在宿主环境中处理这个问题，宿主环境也可以访问这个 prop。
    // 这避免了分配另一个 HostText fiber 并遍历它。
    nextChildren = null;
  } else if (prevProps !== null && shouldSetTextContent(type, prevProps)) { // 之前是文本，之后不是文本
    // If we're switching from a direct text child to a normal child, or to
    // empty, we need to schedule the text content to be reset.
    // 如果我们要从一个直接的文本子节点切换到一个正常的子节点，或者切换为空，
    // 那么我们需要调度要重置的文本内容。
...
function mountIndeterminateComponent(
  ...
  if (_current !== null) {
    ...
    // a new mount, even though an empty version of it already committed.
    // Disconnect the alternate pointers.
    // 不确定组件只有在非并发树中(处于不一致状态)时才会挂载。
    // 我们想把它当作一个新的挂载，即使它已经提交了一个空版本。
    // 断开 alternate 指针。
    _current.alternate = null;
    workInProgress.alternate = null;
    // Since this is conceptually a new fiber, schedule a Placement effect
    // 由于这在概念上是一种新的 fiber，所以安排一个 Placement 效果
    workInProgress.effectTag |= Placement;
  }
...
function bailoutOnAlreadyFinishedWork(
  ...
  if (enableProfilerTimer) {
    ...
    // 不要更新 bailouts 的 "base" 渲染时间。
    stopProfilerTimerIfRunning(workInProgress);
  }

  // Check if the children have any pending work.
  // 检查 children 是否有任何未完成的工作。
  const childExpirationTime = workInProgress.childExpirationTime;
  if (childExpirationTime < renderExpirationTime) {
    // The children don't have any work either. We can skip them.
    // children 也没有工作。我们可以跳过它们。
    // TODO: Once we add back resuming, we should check if the children are
    // a work-in-progress set. If so, we need to transfer their effects.
    // TODO: 一旦我们添加了恢复，我们应该检查孩子们是否是一个正在进行的工作集。如果是，我们需要转移他们的影响。
    return null;
  } else {
    // This fiber doesn't have work, but its subtree does. Clone the child
    // fibers and continue.
    // 这个 fiber 没有工作，但是它的子树有。克隆子 child 并继续。
 ...
 * @param workInProgress
 * @param renderExpirationTime
 * @returns {(Fiber|null)|null|*|Fiber|null|*|Fiber|*|null}
 */
function beginWork(
  current: Fiber | null,
  workInProgress: Fiber,
  ...
  if (current !== null) {
    ...
    if (oldProps !== newProps || hasLegacyContextChanged()) {
      ...
      // 如果 props 或 context 发生了变化，将 fiber 标记为已完成工作。
      // 如果稍后确定 props 相等，则可能取消设置(memo)。
      didReceiveUpdate = true;
