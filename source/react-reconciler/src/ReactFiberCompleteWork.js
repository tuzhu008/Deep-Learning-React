...
if (supportsMutation) {
  ...
  updateHostComponent = function(
    ...
    if (oldProps === newProps) {
      ...
      // 在突变模式下，这对于紧急救援(bailout)来说已经足够了，
      // 因为即使子孙发生了变化，我们也不会触及这个节点。
      return;
    }

    // If we get updated because one of our children updated, we don't
    // have newProps so we'll have to reuse them.
    // 如果我们因为我们的一个孩子更新了而得到更新，我们没有 newProps，所以我们必须重用它们。
    ...
    // 如果更新有效负载指示有更改，
    // 或者有一个新的 ref，我们将其标记为一个更新。所有的工作都是在 commitWork 中完成的。
    if (updatePayload) {
...
function completeWork(
  ...
  switch (workInProgress.tag) {
    ...
    case HostComponent: {
      ...
      const rootContainerInstance = getRootHostContainer(); // 根 dom 对象
