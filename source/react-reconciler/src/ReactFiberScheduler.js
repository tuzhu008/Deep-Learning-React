...
function completeUnitOfWork(workInProgress: Fiber): Fiber | null {
  ...
  // parent fiber.
  // 尝试完成当前的工作单元，然后移动到下一个兄弟节点（sibling）。
  // 如果没有更多的兄弟节点，返回到父 fiber。
  while (true) {
    // The current, flushed, state of this fiber is the alternate.
    // Ideally nothing should rely on this, but relying on it here
    // means that we don't need an additional field on the work in
    // progress.
    // fiber 的 current，flushed，state 是 alternate。
    // 理想情况下不应该依赖于此，但是依赖于此意味着我们在正在进行的工作上的不需要额外的字段。
    ...
    if ((workInProgress.effectTag & Incomplete) === NoEffect) {
      ...
      // 这个 fiber 已完成
      // 记住，我们正在完成这个单元，所以如果它失败了，我们可以找到一个边界。
      // 成功我们为它赋另一个值，失败仍是当前值
      ...
      if (siblingFiber !== null) {
        ...
        // 如果在 returnFiber 还有更多的工作要做，那就接着做。
        return siblingFiber;
      } else if (returnFiber !== null) {
        // If there's no more work in this returnFiber. Complete the returnFiber.
        // 如果在这个 returnFiber 中没有更多的工作，那就完成 returnFiber。
      ...
      if (siblingFiber !== null) {
        // If there is more work to do in this returnFiber, do that next.
        // 如果 returnFiber 还有更多的工作要做，那就接着做。
        return siblingFiber;
      } else if (returnFiber !== null) {
        // If there's no more work in this returnFiber. Complete the returnFiber.
        // 如果在 returnFiber 没有更多的工作了。去完成 returnFiber。
        workInProgress = returnFiber;
        continue;
      } else {
        return null;
