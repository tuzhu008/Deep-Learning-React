...
/**
 * 组件是否自动聚焦
 * 1. 能够自动聚焦的元素类型有限：button、input、select、textarea，
 *  此类元素只需要获取他们的 autoFocus 属性值就可以了
 * 2. 其余的元素都为 false，都不可以自动聚焦
 *
 * @param type
 * @param props
 * @returns {boolean}
 ...
 * @returns {{namespace: *, ancestorInfo}|*}
 */
 ...
 * @param hostContext
 * @param {FiberNode} internalInstanceHandle
 * @returns {Instance}
 */
export function createInstance(
  ...
  if (__DEV__) {
    ...
    validateDOMNesting(type, null, hostContextDev.ancestorInfo); // 验证 tag 标签是否有效
    // 文本子节点
    if (
