...
/**
 * 实现一个 <input> host 组件，
 * 该组件允许设置这些可选的 props: 'checked'、'value'、'defaultChecked' 和 'defaultValue'。
 *
 * 如果没有提供 `checked` 或 `value` (或null/undefined)，
 * 影响选中状态或值的用户操作将触发对元素的更新。
 *
 * 如果它们被提供(并且不是 null/undefined)，渲染的元素将不会触发对元素的更新。
 * 相反，为了更新渲染的元素，这些 props 必须更改。
 *
 * 渲染的元素将初始化为 unchecked(或'defaultChecked')，并带有一个空值(或'defaultValue')。
 *
